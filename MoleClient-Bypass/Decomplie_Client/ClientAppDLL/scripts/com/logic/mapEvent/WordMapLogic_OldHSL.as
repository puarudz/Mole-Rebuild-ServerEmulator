package com.logic.mapEvent
{
   ...
   public class WordMapLogic_OldHSL extends Sprite
   {
      ...
      public function WordMapLogic_OldHSL(map:*)
      {
         super();
      ...
      private function loadXML() : void
      {
         ...
         this.worldXML = XMLInfo.WorldMapXML_OldHSL;
         ...
         for(var i:uint = 0; i < tempList.length(); i++)
         {
            ...
            subList = tempList[i].tip.angleFight.fightObj;
            if(subList.length() > 0)
            {
               tempDecObj = new MapDescriptionObj("摩摩怪大作戰",99);
               tempTipObj.angelFightArr.push(tempDecObj);
            }
            for(j = 0; j < subList.length(); j++)
            {
               tempDecObj = new MapDescriptionObj("* " + subList[j].@descrption,subList[j].@colorNum);
      ...
      private function showTips(CmapObj:MapObj) : void
      {
         ...
         while(mapMC.tips.fighting.textHolder.numChildren > 0)
         {
            mapMC.tips.fighting.textHolder.removeChildAt(mapMC.tips.fighting.textHolder.numChildren - 1);
         }
         var defaultTempY:int = 32;
         var tempY:int = defaultTempY;
         ...
         if(CmapObj.tipObj.angelFightArr.length > 0)
         {
            textY = 0;
            mapMC.tips.fighting.y = tempY;
            mapMC.tips.fighting.visible = true;
            mapMC.tips.fighting.alpha = 0;
            TweenLite.to(mapMC.tips.fighting,0.5,{
               "alpha":1,
               "delay":0.4
            });
            for(i = 0; i < CmapObj.tipObj.angelFightArr.length; i++)
            {
               tempTxt = new TextField();
               tempTxt.width = 145;
               tempTxt.maxChars = 10;
               tempTxt.setTextFormat(textFormat);
               tempTxt.text = CmapObj.tipObj.angelFightArr[i].descrpiton;
               tempTxt.textColor = CmapObj.tipObj.angelFightArr[i].color;
               tempTxt.x = textX;
               tempTxt.y = textY + i * tempTxtDistance;
               mapMC.tips.fighting.textHolder.addChild(tempTxt);
               tempFightingHeight += tempTxtDistance;
            }
            mapMC.tips.fighting.area.height = tempFightingHeight;
            tempY += tempFightingHeight + tempDistance;
         }
         else
         {
            mapMC.tips.fighting.visible = false;
         }
