package com.module.home
{
   ...
   public class GuestList
   {
      ...
      private static var instance:GuestList = null;
      
      public var ID:*;
      
      public var ListUI:MovieClip;
      ...
      public static function getInstance() : GuestList
      {
         return instance = instance || new GuestList();
      }
      ...
      private function loadUI() : void
      {
         if(!this.ListUI)
         {
            this.mcloader = new MCLoader("module/home/GuestList.swf",MainManager.getGameLevel(),1,"正在打開家園動態...");
      ...
      public function showGuest(e:MouseEvent = null) : void
      {
         ...
         {
            GV.onlineSocket.addEventListener(GetHomeInfoRes.GET_HOME_GUEST,this.showVisitor);
            GetHomeInfoReq.HomeGuest(hotID());
         }
         else
         {
      ...
      {
         var i:uint = 0;
         GV.onlineSocket.removeEventListener(GetHomeInfoRes.GET_HOME_GUEST,this.showVisitor);
      ...
      {
         var temp:Object = new this.Man();
         temp.visible = false;
         temp.prev_pet.visible = false;
         temp.id = manInfo.UserID;
         temp.name = "mc" + temp.id;
         temp.userName.text = manInfo.Nick;
         if(GF.BT(manInfo.Flag,6))
         {
            temp.water_muck.gotoAndStop(6);
         }
         else if(GF.BT(manInfo.Flag,5))
         {
         ...
         else if(GF.BT(manInfo.Flag,1))
         {
            temp.water_muck.gotoAndStop(3);
         }
         else if(GF.BT(manInfo.Flag,2))
         {
            temp.water_muck.gotoAndStop(int(Math.random() * 2 + 2));
         }
         else
         {
            temp.water_muck.gotoAndStop(1);
         }
