package com.greensock
{
   ...
   public class TweenMax extends TweenLite implements IEventDispatcher
   {
      ...
      {
         if(Boolean(toVars.isGSVars))
         {
            toVars = toVars.vars;
         }
         if(Boolean(fromVars.isGSVars))
         {
            fromVars = fromVars.vars;
         }
         toVars.startAt = fromVars;
         if(Boolean(fromVars.immediateRender))
         {
      ...
      override public function renderTime(time:Number, suppressEvents:Boolean = false, force:Boolean = false) : void
      {
         ...
         if(this._repeat != 0)
         {
            ...
            if((this._cyclesComplete = this.cachedTotalTime / cycleDuration >> 0) == this.cachedTotalTime / cycleDuration && this._cyclesComplete != 0)
            {
               --this._cyclesComplete;
            }
            repeated = Boolean(prevCycles != this._cyclesComplete);
            if(isComplete)
            {
