package fl.motion
{
   ...
   public class MatrixTransformer3D
   {
      ...
      {
         var vecI:Vector3D = null;
         var n:Array = null;
         var j:int = 0;
         var vec3D:Vector.<Number> = null;
         var vecJ:Vector3D = null;
         ...
         for(var i:int = 0; i < 3; i++)
         {
            if(i != axis)
            {
               vecI = getVector(mat,i);
               n = new Array(3);
               for(j = 0; j < 3; j++)
               {
                  vecJ = getVector(tempMat,j);
               ...
               vec3D = Vector.<Number>(getRawDataVector(mat));
               vec3D[i * 4] = vecI.x;
               vec3D[i * 4 + 1] = vecI.y;
               vec3D[i * 4 + 2] = vecI.z;
               vec3D[i * 4 + 3] = vecI.w;
